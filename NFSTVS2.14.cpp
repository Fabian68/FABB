...
string NombreAvecEspaceMillier(long long int const & Nombre) {
...
bool AttaquePossible(int JoueurDefenseur) {
	return(PersoCarac[JoueurDefenseur][STATUT] == EnVie);
}
...
void Attaque(int Degat,int Defenseur,int Attaquant) {
	if (AttaquePossible(Defenseur)) {
		...
		if (tiragePersoCarac(Defenseur, DEVIATION)) {
			cout << endl << " " << Personnage[Defenseur] << "  DEVIE l'attaque ! " << endl;
	...
	switch(choix) {
		...
		case 0:
			...
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && EquipeEnVie(TeamEnnemie(Joueur)) == true) {
				...
				PersoCarac[Joueur][AGILITE] *= static_cast< int>(((1.0 + (3 * Tour) / 100)));
			}
			PersoCarac[Joueur][MANA]++;
			break;
		case 1:
...
void InitialiserUnJoueur(string  NomPersonnage,int PersonnageID,int AGILITE_ID,int FORCE_ID,int VITALITE_ID,int ARME_ID,int CHANCEDOUBLEATTAQUE_ID,int HABILETE_ID,int REDUCTION_ID,int ANIMAL_ID=rand_a_b(0,2),int BLOCAGE_ID=0,int DEVIATION_ID=1,int RICOCHET_ID=1) {
...
double MultiplicateurCaracXp(int Joueur) {
	return (1 + (PersoCarac[Joueur][CHANCEDOUBLEATTAQUE] / 4.0 + PersoCarac[Joueur][HABILETE] / 3.0 + PersoCarac[Joueur][REDUCTION] * 2.0 + PersoCarac[Joueur][BLOCAGE] * 1.5 + PersoCarac[Joueur][DEVIATION] * 3.0 + PersoCarac[Joueur][RICOCHET]) / 100.0);
}
...
bool InitialiserEXP(const string& fichier) {
	...
	for(int i=0;i<LVLEXP;i++) {
		...
		UnEntierTresTresLong= static_cast<long long int>(30*i+10*pow(1.00342,i)-30+i*i);
...
void affichageUnJoueur(int IndiceJoueur) {
	...
	cout<<"\tReduction Degats :"<<PersoCarac[IndiceJoueur][REDUCTION]<<"%"<< "\tChance Blocage :" << PersoCarac[IndiceJoueur][BLOCAGE] << "%" << endl<<endl;
}
...
void InitialiserToutJoueurs() {
	InitialiserUnJoueur("FABIAN",FABIAN,7,7,4,TRUGDOR,20,20,0,OFFENSIF,0,3,17);
	InitialiserUnJoueur("THOMAS",THOMAS,1,4,20,VENTRE,20,80,80,DEFENSIF,80,8,10);
	InitialiserUnJoueur("CLOE",CLOE,11,5,3,BATON,25,10,0, DEFENSIF,0,2,7);
	...
	InitialiserUnJoueur("JB",JB,7,7,7,JBI,1,1,70, DEFENSIF,7,7,7);
	InitialiserUnJoueur("SEBASTIEN",SEBASTIEN,15,2,2,SPARTAN,30,15,0, OFFENSIF,1,1,5);
	InitialiserUnJoueur("FLO",FLO,2,5,13,ALACOOL,10,30,10, DEFENSIF);
	InitialiserUnJoueur("BRYAN",BRYAN,9,9,1,DAGUES,33,33,-33, OFFENSIF,0,3,7);
	...
	InitialiserUnJoueur("ISIDORE", ISIDORE, 15, 1, 5, ISIDOOR, 10, 10, 30, DEFENSIF,10,4,6);
	...
	switch(Zone) {
		case 1:
				...
				switch(Niveau) {
					...
					case 2:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=5;InitialiserUnJoueur("LAPIN ADULTE" ,S3,8,2,2,LAPIN,15,0,10, OFFENSIF,10);
					...
					case 5:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=15;InitialiserUnJoueur("Lapin Agile ",S3,15,1,1,LAPIN,50,50,0, OFFENSIF,0,0,10);
					...
					case 6:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=20;InitialiserUnJoueur("Lapin Agile" ,S3,15,1,1,LAPIN,50,50,0, OFFENSIF,0,0,10);
					...
					case 10:
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=50;InitialiserUnJoueur("Dieu Lapin ",S4,3,1,1,LAPIN,5,0,0, OFFENSIF,0,0,30);
		...
		case 2:
			...
			switch(Niveau) {
					case 1:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=55;InitialiserUnJoueur("CHEF LOUP ",S3,16,16,8,LAPIN,20,20,10, DEFENSIF,10);
					...
					case 2:
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=75;InitialiserUnJoueur("LAPIN DIVIN ",S5,20,5,5,LAPIN,50,50,50, DEFENSIF,0,0,30);
						break;
					case 3:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
					...
					case 4:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=100;InitialiserUnJoueur("Rattata ",S3,10,10,10,LAPIN,25,25,25, OFFENSIF,25,3,25);
					...
					case 6:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=125;InitialiserUnJoueur("Lapin Agile ",S3,15,1,1,LAPIN,50,50,0, OFFENSIF,0,0,5);
		...
		case 3:
			...
			switch(Niveau) {
					case 1:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 1 ",S1,25,5,5,LAPIN,10,10,0, OFFENSIF,0,0,10);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 2 ",S2,25,5,5,LAPIN,10,10,0, OFFENSIF,0,0,10);
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 3  ",S4,25,5,5,LAPIN,10,10,0, OFFENSIF,0,0,10);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 4  ",S5,25,5,5,LAPIN,10,10,0, OFFENSIF,0,0,10);
						break;
					case 2:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
					...
					case 4:
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=250;InitialiserUnJoueur(" DRAGON JB ",S5,7,7,7,JBI,7,7,7,DEFENSIF,7,7,7);
						break;
					case 5:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
		...
		case 4:
			...
			case 5:
				...
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 525; InitialiserUnJoueur("GRAND DRAGON SEB ", S2, 30, 30, 30, SPARTAN, 30, 30, 30, OFFENSIF, 0,0,30);
			...
			case 7:
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 558; InitialiserUnJoueur("GRAND DRAGON SAGE FABIAN ", S4, 30, 30, 30, TRUGDOR, 30, 30, 30,OFFENSIF,10,10,70);
		...
		case 5:
			...
			case 1:
				...
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 700; InitialiserUnJoueur("EASYDOOR  ", S1, 70, 10, 300, ISIDOOR, 50, 50, 75, DEFENSIF,75,5,5);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 750; InitialiserUnJoueur(" PERE DE NICO TRES FURIEUX  ", S1, 33, 33, 330, DEMO, 33, 33,33, DEFENSIF,33,7,3);
				break;
			case 3:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 800; InitialiserUnJoueur(" VICTOR HUGOT  ", S1, 30, 70, 250, KUNGFU, 100, 50, 90, DEFENSIF,90,10,1);
			...
			case 5:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 900; InitialiserUnJoueur(" BRYAN IN THE KITCHEN  ", S1, 44, 44, 100, DAGUES, 75, 75, 0, OFFENSIF,0,0,7);
			...
			case 7:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1000; InitialiserUnJoueur(" SEB LA FRITE  ", S1, 70, 10, 150, SPARTAN, 80, 20, 0, OFFENSIF,0,0,12);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1050; InitialiserUnJoueur(" JB LE SOLIDE  ", S1, 70, 10, 500, JBI, 50, 50, 99, DEFENSIF,99,9,9);
				break;
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
			...
			case 10:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1150; InitialiserUnJoueur(" MAELLE LA TYRANIQUE  ", S1, 40, 40, 650, AMAELLE, 50, 50, 50, DEFENSIF,50);
				break;
			}
			break;
		case 6:
			...
			case 1:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1200; InitialiserUnJoueur("Elite *  ", S1, 3, 10, 10, ELITE, 10, 10, 10, DEFENSIF,10);
			...
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1210; InitialiserUnJoueur("Elite * *  ", S1, 5, 12, 12, ELITE, 20, 20, 20,DEFENSIF,20);
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1210; InitialiserUnJoueur("Chasseur * ", S4, 8, 8, 1, CHASSEUR, 2, 2, 0,OFFENSIF,50,10,10);
			...
			case 3:
				...
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1220; InitialiserUnJoueur("Chasseur * * 1  ", S3, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF,50,10,10);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1220; InitialiserUnJoueur("Chasseur * * 2 ", S4, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF,50,10,10);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1220; InitialiserUnJoueur("Chasseur * * 3 ", S5, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF,50,10,10);
				break;
			case 4:
				cout << endl << "Niveau " << Niveau << " : " << endl;
			...
			case 6:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1250; InitialiserUnJoueur("ELITE * * * ", S1, 7, 16, 16, ELITE, 30, 30, 30,DEFENSIF,30);
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1250; InitialiserUnJoueur("Chasseur * * * ", S4, 12, 12, 2, CHASSEUR, 8, 8, 2,OFFENSIF,50,10,10);
			...
			case 7:
				...
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1260; InitialiserUnJoueur("Chasseur * * * * ", S5, 16, 16, 3, CHASSEUR, 16, 16, 4,OFFENSIF,50,10,10);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1270; InitialiserUnJoueur("Chasseur 1 * * * * * ", S4, 20, 20, 5, CHASSEUR,32, 32, 8,OFFENSIF,50,10,10);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1270; InitialiserUnJoueur("Chasseur 2 * * * * * ", S5, 20, 20, 5, CHASSEUR, 32, 32, 8,OFFENSIF,50,10,10);
				break;
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				...
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1280; InitialiserUnJoueur("Elite 1 * * * * ", S3, 10, 20, 20, ELITE, 40, 40, 40,DEFENSIF,40);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1280; InitialiserUnJoueur("Elite 2 * * * * ", S4, 10, 20, 20, ELITE, 40, 40, 40,DEFENSIF,40);
		...
		case 7:
			...
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1310; InitialiserUnJoueur("Elite * * * * *  ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1310; InitialiserUnJoueur("Elite * * * * * 1 ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
			...
			case 3:
				...
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1320; InitialiserUnJoueur("Chasseur * * * * * 1  ", S3, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
			...
			case 5:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1340; InitialiserUnJoueur("Elite 1 * * * * * ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1340; InitialiserUnJoueur("Elite 2 * * * * * ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1340; InitialiserUnJoueur("Chasseur  * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
			...
			case 6:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1350; InitialiserUnJoueur("ELITE * * * * * 1 ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1350; InitialiserUnJoueur("ELITE * * * * * 2", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1350; InitialiserUnJoueur("ELITE * * * * * 3 ", S3, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1350; InitialiserUnJoueur("ELITE * * * * * 4 ", S4, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1350; InitialiserUnJoueur("Chasseur * * * * ", S5, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				break;
			case 7:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				...
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1360; InitialiserUnJoueur("Chasseur 1 * * * * * ", S3, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1360; InitialiserUnJoueur("Chasseur 2 * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1360; InitialiserUnJoueur("Chasseur * * * *  * ", S5,25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
			...
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1380; InitialiserUnJoueur("Elite 4 * * * * * ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1380; InitialiserUnJoueur("Elite 3 * * * * * ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1380; InitialiserUnJoueur("Elite 1 * * * * * ", S3, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1380; InitialiserUnJoueur("Elite 2 * * * * * ", S4, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1380; InitialiserUnJoueur("Elite 5 * * * * * ", S5, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF,50);
				break;
			case 10:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				...
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1390; InitialiserUnJoueur("Chassaur 1 * * * * * ", S2, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1390; InitialiserUnJoueur("Chassaur 2 * * * * * ", S3, 25, 25, 50, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1390; InitialiserUnJoueur("Chassaur 3 * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1390; InitialiserUnJoueur("Chassaur 4 * * * * * ", S5, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF,50,10,10);
				break;
			}
			break;
		case 8:
...
void AfficherMiseAjour() {
	...
	cout << endl << "--> V.12 Bugs varies resolus ." << endl;
	cout << endl << "--> V.12 Zone 5 plus compliquer ." << endl;
	cout << endl << "--> V.12 Nerf de salim en fin de jeu." << endl;
	cout << endl << "--> V.12 Les orbes de vies sont meilleur que les autres orbes ." << endl;
	cout << endl << "--> V.13 Legeres Optimisations du jeu ." << endl;
	cout << endl << "--> V.13 Rajouter du blocage,une attaque bloquer divise les degats par 2 ." << endl;
	cout << endl << "--> V.14 Resolutions de bug divers ." << endl;
	cout << endl << "--> V.14 Rajout de la mecaniquer deviation(renvoie attaque vers l'ennemi) ." << endl;
	cout << endl << "--> V.14 Rajoue mecanique attaque qui ricoche ." << endl;
	cout << endl << "--> V.14 Experience accru sur les personnages tank en particulier ." << endl;
