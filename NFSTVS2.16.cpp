...
void AttaquerMaelle(int Joueur) {
	...
	case 1:
		...
		DEGATS = static_cast<int>(ceil(frand_a_b(0.33, 1.33)*PersoCarac[Joueur][FORCE]));
		Attaque(DEGATS, Joueur, Joueur);
		for (int i = 0; i < 3 && EquipeEnVie(TeamEnnemie(Joueur)) == true; i++) {
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && PersoCarac[Joueur][STATUT] == EnVie) {
				...
				DEGATS = static_cast<int>(ceil(frand_a_b((i*1.0 + 1.0)*0.05, (i*1.0 + 1.0)*0.15)))*PersoCarac[Joueur][FORCE];
				Attaque(DEGATS, Joueur, Joueur);
...
void AttaquerVENTRE(int Joueur) {
	...
	switch(choix) {
		...
		case 0:
			...
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
				cout<<Personnage[Joueur]<<" pete un gros coup , ca asphyxie tout les ennemis ! "<<endl;
					...
					DEGATS = static_cast<int>(ceil(frand_a_b(0.01,0.06)*PersoCarac[Joueur][VIE]));
		...
		case 1:
			...
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
				cout<<" DOUBLE saut ! "<<endl;
					...
					DEGATS = static_cast<int>(ceil(frand_a_b(0.25,1.0)*PersoCarac[Joueur][FORCE]));
					Attaque(DEGATS,Equipe[TeamEnnemie(Joueur)][PREMIER],Joueur);
					...
					DEGATS = static_cast<int>(ceil(frand_a_b(0.75,1.0)*PersoCarac[Joueur][FORCE]));
					Attaque(DEGATS,Equipe[TeamEnnemie(Joueur)][TROISIEME],Joueur);
		...
		case 2:
			...
			for(int i=PREMIER;i<=CINQUIEME;i++) {
				soins = static_cast<int>(ceil(frand_a_b(0.33,0.63)*PersoCarac[Joueur][FORCE]));
				Soins(TeamTMP,i,soins,Joueur);
			}
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
				...
				for(int i=PREMIER;i<=CINQUIEME;i++) {
					soins = static_cast<int>(ceil(frand_a_b(0.22,0.44)*PersoCarac[Joueur][FORCE]));
					Soins(TeamTMP,i,soins,Joueur);
				}
...
void AttaquerDAGUES(int Joueur) {
	...
	DEGATS = static_cast<int>(ceil(frand_a_b(0.1, 0.30)*PersoCarac[Joueur][FORCE]*PersoCarac[Joueur][DEGATCRITIQUE]/100.0));
	Attaque(DEGATS, CibleAlea(Joueur), Joueur);
	switch(choix) {

		case 0:
			cout<<Personnage[Joueur]<<" Premiere Dague ! "<<endl;
			DEGATS = static_cast<int>(ceil(frand_a_b(0.4,0.6)*PersoCarac[Joueur][FORCE]));
			Attaque(DEGATS,CibleProche(Joueur),Joueur);
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
			...
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
				cout<< " Saignement !  "<<endl;
				DEGATS = static_cast<int>(ceil(frand_a_b(0.1,0.15)*PersoCarac[Joueur][FORCE]));
				Attaque(DEGATS,CibleProche(Joueur),Joueur);
				PersoCarac[Joueur][MANA]++;
			}
			PersoCarac[Joueur][MANA]++;
		...
		case 2:
			...
			for(int i=0;i<3&&EquipeEnVie(TeamEnnemie(Joueur))==true;i++) {
				...
				if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&EquipeEnVie(TeamEnnemie(Joueur))==true) {
					...
					DEGATS = static_cast<int>(ceil(frand_a_b(0.075,0.6)*PersoCarac[Joueur][FORCE]));
					Attaque(DEGATS,CibleAlea(Joueur),Joueur);
				}
			cout<<endl;
			}
...
void AttaquerJBI(int Joueur) {
	...
	switch(choix) {
		...
		case 0:
			...
			Attaque(static_cast<int>((Tour*(Tour+1))*PersoCarac[Joueur][LVL]/100),Joueur,Joueur);
			PersoCarac[Joueur][MANA]++;
			break;
		case 1:
			cout<<Personnage[Joueur]<<" danse ! "<<endl;
			Attaque(static_cast<int>((2*Tour*(Tour + 1))*PersoCarac[Joueur][LVL]/50), Joueur,Joueur);
			PersoCarac[Joueur][MANA]++;
			break;
		case 2:
			cout<<Personnage[Joueur]<<" a le swag ! "<<endl;
			Attaque(static_cast<int>((3*Tour*(Tour + 1))*PersoCarac[Joueur][LVL]/10), Joueur,Joueur);
			PersoCarac[Joueur][MANA]+=2;
			break;
		case 3:

			cout<<Personnage[Joueur]<<" casse le téléphone de "<<Personnage[rand_a_b(0,NBJoueurs)]<<endl;
			Attaque(static_cast<int>((4*Tour*(Tour + 1))*PersoCarac[Joueur][LVL]/1),Joueur,Joueur);
			PersoCarac[Joueur][MANA]+=3;
			break;
	}
...
void InitialiserToutJoueurs() {
	...
	InitialiserUnJoueur("VICTOR", VICTOR, 1,18,1, KUNGFU, 10, 80, 60, DEFENSIF,50,10,10,80);
	...
	InitialiserUnJoueur("BERSERKER", BERSERKER, 10, 6, 4, ABERSERKER, 0, 0, 50, OFFENSIF, 50, 5, 3);
}
	...
	switch(Zone) {
		...
		case 5:
			...
			case 1:
				...
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 700; InitialiserUnJoueur("EASYDOOR  ", S1, 40, 10, 300, ISIDOOR, 50, 50, 75, DEFENSIF,75,5,5,50);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
			...
			case 6:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 950; InitialiserUnJoueur(" FLO D'ENFER  ", S1, 5, 45, 450, ALACOOL, 10, 80, 20, DEFENSIF);
				break;
			case 7:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
...
void AfficherMiseAjour() {
	cout<<endl<<"* * * MISE A JOUR 2.16 * * * "<<endl;
	cout<<endl<<"--> Gros Buff Flo ."<<endl;
	cout << endl << "--> Degats ultime viktor fortement acru, personnage fortement buff ." << endl;
	cout << endl << "--> 20% d'exp gagner en moins ." << endl;
	cout << endl << "--> 1% d'xp gagner en plus par tour jouer dans une partie ." << endl;
	cout << endl << "--> Nicolas possede une nouvelle attaque qui se déclanche quand le combat est trop long ." << endl;
	cout << endl << "--> Bryan nouvel attaque." << endl;
	cout << endl << "--> Nerf degats thomas." << endl;
	cout << endl << "--> Nouvelle attaque Victor." << endl;
