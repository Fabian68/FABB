...
long long int PersoCarac[NBJoueurs][30];
int Equipe[2][5];
int Tour;
...
void AfficherPosition(int position) {
	...
	if(PersoCarac[Equipe[TEAM1][position]][STATUT]==EnVie) {
		cout<<Personnage[Equipe[TEAM1][position]]<<" ("<< to_string(PersoCarac[Equipe[TEAM1][position]][LVL])<<")" <<"\t:";Colorier(VERT);cout<<PersoCarac[Equipe[TEAM1][position]][VIE];Colorier(NORMAL);cout<<"\\";Colorier(VERT);cout<<PersoCarac[Equipe[TEAM1][position]][VITALITE]*10<<"\t        ";
		Colorier(NORMAL);
	}
	else {
		Colorier(ROUGE);
		cout<<Personnage[Equipe[TEAM1][position]]<< " (" << to_string(PersoCarac[Equipe[TEAM1][position]][LVL]) << ")" <<"\t:"<<PersoCarac[Equipe[TEAM1][position]][VIE]<<"\\"<<PersoCarac[Equipe[TEAM1][position]][VITALITE]*10<<"\t        ";
		Colorier(NORMAL);
	}
	if(PersoCarac[Equipe[TEAM2][position]][STATUT]==EnVie) {
		cout<<Personnage[Equipe[TEAM2][position]]<< " (" << to_string(PersoCarac[Equipe[TEAM2][position]][LVL]) << ")" <<"\t:";Colorier(VERT);cout<<PersoCarac[Equipe[TEAM2][position]][VIE];Colorier(NORMAL);cout<<"\\";Colorier(VERT);cout<<PersoCarac[Equipe[TEAM2][position]][VITALITE]*10<<"\t        ";
		Colorier(NORMAL);
	}
	else {
		Colorier(ROUGE);
		cout<<Personnage[Equipe[TEAM2][position]]<< " (" << to_string(PersoCarac[Equipe[TEAM2][position]][LVL]) << ")" <<"\t:"<<PersoCarac[Equipe[TEAM2][position]][VIE]<<"\\"<<PersoCarac[Equipe[TEAM2][position]][VITALITE]*10<<"\t        ";
		Colorier(NORMAL);
	}
	cout<<endl;
}
...
bool TeamPerdue(int NumeroTeam) {
...
void LootOrbes(int J) {
	...
	if(choix<100) {
		...
		if(d==1&&PersoCarac[J][ORBEVIE2] != 2) {
			PersoCarac[J][ORBEVIE2]=2;
	...
	if(choix==1) {
		...
		if(d==1&& PersoCarac[J][ORBEVIE3] != 3) {
			PersoCarac[J][ORBEVIE3]=3;
...
void Soins(int team, int position, int soins, int joueur) {
	...
	if (soinsPossible(team, position)) {
		...
		cout << Personnage[Equipe[team][position]] << " recupere "; Colorier(VERT); cout << soins << " vies ." << endl;
		Colorier(NORMAL);
		Sleep(300);
	}
	cout << endl;
...
bool AttaquePossible(int Joueur, int Position) {
	if (TeamPerdue(TEAM1) && TeamPerdue(TEAM2)) {

		if (PersoCarac[Joueur][EQUIPE] == TEAM1) {
			return(PersoCarac[Equipe[TEAM2][Position]][STATUT] == EnVie);
		}
		else {
			return(PersoCarac[Equipe[TEAM1][Position]][STATUT] == EnVie);
		}
	}
	else {
		return false;
...
void Attaque(int Degat,int Defenseur,int Attaquant) {
	...
	int Degats;
	int TeamTMP;
	if (PersoCarac[Defenseur][EQUIPE] == TEAM1) {
		TeamTMP = TEAM1;
	}
	else {
		TeamTMP = TEAM2;
	...
	cout<<Personnage[Defenseur]<<" recois ";Colorier(ROUGE);cout<<Degat;Colorier(NORMAL);cout<<" degats . "<<endl;
	Colorier(NORMAL);
	Sleep(300);
	if (rand_a_b(0, 101) < 33 &&PersoCarac[Defenseur][ANIMAL] == DEFENSIF) {
		...
		Soins(TeamTMP, PositionJoueur(Defenseur), Degats, Defenseur);
		Sleep(300);
	}
	if(!EstEnVie(Defenseur)) {
		...
		if (AttaquePossible(Attaquant, CibleProche(Attaquant))) {
...
void AttaquerISIDORE(int Joueur) {
	...
	switch(choix) {
		case 0:
			...
			if(tiragePersoCarac(Joueur,HABILETE)&&TeamPerdue(TeamEnnemie(Joueur))==true) {
		...
		case 1:
			...
			for (double i = 0; i < 1; i += 0.01) {
				if(tiragePersoCarac(Joueur, HABILETE)) {
					soins = static_cast<int>(ceil(frand_a_b(i, i + 0.01)*PersoCarac[Joueur][FORCE]));
					Soins(TEAM1, PositionJoueur(Joueur), soins, Joueur);
				}
			}
			PersoCarac[Joueur][MANA]--;
			break;
		case 2:
			...
			for (int i = PREMIER; i <= CINQUIEME; i++) {
				PersoCarac[Equipe[TEAM2][i]][REDUCTION] = PersoCarac[Equipe[TEAM2][i]][REDUCTION] - static_cast<long long int>(1);
			}
		...
		case 3:
			...
			PersoCarac[Cible][REDUCTION] = PersoCarac[Cible][REDUCTION] - static_cast<long long int>(20);
...
void AttaquerSPARTAN(int Joueur) {
	...
	switch(choix) {
		...
		case 0:
			...
			for(int i=0;i<11&&TeamPerdue(TeamEnnemie(Joueur))==true;i++,inc+=0.01) {

				DEGATS = static_cast<int>(ceil(frand_a_b(0.1,0.2+inc)*PersoCarac[Joueur][FORCE]));
				Attaque(DEGATS,CibleProche(Joueur),Joueur);
				if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&TeamPerdue(TeamEnnemie(Joueur))==true) {
			...
			PersoCarac[CibleProche(Joueur)][REDUCTION] = PersoCarac[CibleProche(Joueur)][REDUCTION] - static_cast<long long int>(1);
			inc = 0;
			break;
		case 1:
		...
		case 3:
			...
			if(AttaquePossible(Joueur ,PREMIER)) {
			...
			if(AttaquePossible(Joueur ,DEUXIEME)) {
	...
	switch(choix) {
		...
		case 1:
				if(tiragePersoCarac(Joueur,HABILETE)) {
					...
					PersoCarac[K][REDUCTION]= PersoCarac[K][REDUCTION]- static_cast<long long int>(6);
					
				}
				else {
					int K=CibleAlea(Joueur);
					cout<<endl<<Personnage[Joueur]<<"Fais un petit calin a "<<Personnage[K]<<endl;
					PersoCarac[K][REDUCTION] = PersoCarac[K][REDUCTION] - static_cast<long long int>(3);
				}
			PersoCarac[Joueur][MANA]++;
			break;
		case 2:
...
void AttaquerMaelle(int Joueur) {
	...
	case 0:
		...
		Cible = CibleAlea(Joueur);
		for (int i = 0; i < 5 && TeamPerdue(TeamEnnemie(Joueur)) == true; i++) {
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && PersoCarac[Cible][STATUT] == EnVie && PersoCarac[Joueur][STATUT] == EnVie) {
				...
				PersoCarac[CibleProche(Joueur)][REDUCTION] = PersoCarac[Cible][REDUCTION] - static_cast<long long int>(1);
				inc += 0.2;
			}
		}
		PersoCarac[Joueur][MANA]++;
	...
	case 1:
		...
		Attaque(DEGATS, Joueur, Joueur);
		for (int i = 0; i < 3 && TeamPerdue(TeamEnnemie(Joueur)) == true; i++) {
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && PersoCarac[Joueur][STATUT] == EnVie) {
				...
				Attaque(DEGATS, Joueur, Joueur);
				PersoCarac[Joueur][FORCE] = static_cast<long long int>(PersoCarac[Joueur][FORCE] * 1.1);
				PersoCarac[Joueur][AGILITE] = static_cast<long long int>(PersoCarac[Joueur][AGILITE] * 1.1);
			}
		}
		PersoCarac[Joueur][MANA]--;
		break;
	case 2:
		...
		for (int i = 0; i < 100 && TeamPerdue(TeamEnnemie(Joueur)) == true; i++) {
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && PersoCarac[Joueur][STATUT] == EnVie) {

				DEGATS = static_cast<int>(ceil(frand_a_b(i*0.001, i*0.002)*PersoCarac[Joueur][FORCE]));
				Attaque(DEGATS, Joueur, Joueur);
				PersoCarac[Joueur][FORCE] = static_cast<long long int>(PersoCarac[Joueur][FORCE] * 1.001);
				PersoCarac[Joueur][AGILITE] = static_cast<long long int>(PersoCarac[Joueur][AGILITE] * 1.001);
			}
		}
		PersoCarac[Joueur][MANA] -= 2;
		break;
...
void AttaquerDEMO(int Joueur) {
	...
	case 1:
		...
		DEGATS = static_cast<int>(ceil(frand_a_b(1,2)*PersoCarac[Joueur][FORCE]));
		Soins(TEAM1, PositionJoueur(Joueur), DEGATS, Joueur);
	...
	case 3:
		...
		if(t==false) {
			...
			Attaque(DEGATS, CibleAlea(Joueur), Joueur);
			if(AttaquePossible(Joueur,CibleProche(Joueur))) {
		...
		case 0:
			...
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&TeamPerdue(TeamEnnemie(Joueur))==true) {
				cout<<Personnage[Joueur]<<" pete un gros coup , ca asphyxie tout les ennemis ! "<<endl;
					for(int i=PREMIER;i<=CINQUIEME&&TeamPerdue(TeamEnnemie(Joueur))==true;i++) {

						if(AttaquePossible(Joueur ,i)) {
...
void AttaquerTRUGDOR(int Joueur) {
	...
	switch(choix) {
		...
		case 0:
			cout<<Personnage[Joueur]<<" flèche dans le genoue ! "<<endl;
			...
			if (tiragePersoCarac(Joueur, CHANCEDOUBLEATTAQUE) && TeamPerdue(TeamEnnemie(Joueur)) == true) {
				cout << " Attention j'ai fais de la muscu !  " << endl;
				PersoCarac[Joueur][FORCE] *= static_cast<long long int>(((1.0 + (3 * Tour) / 100)));
				PersoCarac[Joueur][AGILITE] *= static_cast<long long int>(((1.0 + (3 * Tour) / 100)));
			}
			PersoCarac[Joueur][MANA]++;
			break;
		case 1:
			...
			if(tiragePersoCarac(Joueur,HABILETE)){
				...
				cout<<endl<<"Des joueurs ont bouge . "<<endl;
			}
			PersoCarac[Joueur][MANA]--;
			break;
		case 2:
			cout<<Personnage[Joueur]<<" Envoie une flèche  ! "<<endl;
		...
		case 3:
			...
			for(int i=0;i<21&&TeamPerdue(TeamEnnemie(Joueur))==true;i++) {
			...
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&TeamPerdue(TeamEnnemie(Joueur))==true) {
				...
				for(int i=0;i<11&&TeamPerdue(TeamEnnemie(Joueur))==true;i++) {
						DEGATS = static_cast<int>(ceil(frand_a_b(0.14,0.28)*PersoCarac[Joueur][FORCE]));
						Attaque(DEGATS,CibleAlea(Joueur),Joueur);
						if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&TeamPerdue(TeamEnnemie(Joueur))==true) {
							cout<<" DOUBLE FLECHE ! "<<endl;
...
void AttaquerNINI(int Joueur) {
	...
	switch(choix) {
		...
		case 1:
			...
			for(int i=p;i<5&&TeamPerdue(TeamEnnemie(Joueur))==true;i++) {
		...
		case 2:
			...
			for(int i=0;i<(rand_a_b(25,35))&&TeamPerdue(TeamEnnemie(Joueur))==true;i++) {
		...
		case 3:
			...
			Soins(TeamTMP,PositionJoueur(Joueur),soins,Joueur);
			for(int i=PREMIER;i<=CINQUIEME;i++) {
				PersoCarac[Equipe[TeamTMP][i]][FORCE]= static_cast<long long int>(PersoCarac[Equipe[TeamTMP][i]][FORCE] * 1.1);
				PersoCarac[Equipe[TeamTMP][i]][AGILITE]= static_cast<long long int>(PersoCarac[Equipe[TeamTMP][i]][AGILITE] * 1.1);
			}
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)) {
...
void AttaquerSALIM(int Joueur) {
	...
	switch(choix) {
		...
		case 0:
			...
			DEGATS = static_cast<int>(ceil(0.0001*PersoCarac[Joueur][VIE]*PersoCarac[Joueur][FORCE]));
			Attaque(DEGATS,CibleProche(Joueur),Joueur);
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)) { 
				cout<<Personnage[Joueur]<<" enracinage ! "<<endl;
				Soins(TEAM1,PositionJoueur(Joueur), static_cast<int>(PersoCarac[Joueur][VIE]*0.01),Joueur);
		...
		case 1:
			...
			Soins(TEAM1,PositionJoueur(CibleFaible(30)),soins,Joueur);
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)) { 
				PersoCarac[Joueur][REDUCTION]++;
				cout<<Personnage[Joueur]<<" soin de la foret ! "<<endl;
				soins = static_cast<int>(ceil(frand_a_b(0.6,1.8)*PersoCarac[Joueur][FORCE]));
		...
		case 2:
			...
			for(int i = PREMIER;TeamPerdue(TEAM2)&&i<=CINQUIEME;i++) {
				soins = static_cast<int>(ceil(frand_a_b(0.05*i*i,0.1*i*i)*PersoCarac[Joueur][FORCE]));
				Soins(TEAM1,PositionJoueur(CibleFaible(30)),soins,Joueur);
			}
			if(tiragePersoCarac(Joueur,CHANCEDOUBLEATTAQUE)&&TeamPerdue(TEAM2)) { 
				for(int i = PREMIER;!TeamPerdue(TEAM2)&&i<=CINQUIEME;i++) {
					if(TeamPerdue(TEAM2)) {
...
void AttaquerTELLURIQUE(int Joueur) {
	...
	case 1:
		...
		spread1 = static_cast<int>(PersoCarac[Equipe[TEAM1][TROISIEME]][VIE]/250);
		spread2 = static_cast<int>(PersoCarac[Equipe[TEAM1][DEUXIEME]][VIE] / 250+spread1);
		spread3 = static_cast<int>(PersoCarac[Equipe[TEAM1][QUATRIEME]][VIE] / 250+spread1);
		if (AttaquePossible(Joueur, TROISIEME)) {
			DEGATS = static_cast<int>(ceil(frand_a_b(0.01, 0.1)*PersoCarac[Joueur][FORCE]));
	...
	case 2:
		...
		if (AttaquePossible(Equipe[TEAM1][TROISIEME], TROISIEME)) {
...
void AttaquerELITE(int Joueur) {
	...
	case 0:
		...
		for (int i = 0; i < 10 && TeamPerdue(TeamEnnemie(Joueur)) == true; i++) {
...
void AttaquerGROGNARD(int Joueur) {
	...
	case 0:
		...
		for (int i = 0; i < 20 && TeamPerdue(TeamEnnemie(Joueur)) == true; i++,tic+=0.1) {
		...
		if (TeamPerdue(TeamEnnemie(Joueur))) {
			DEGATS = static_cast<int>(ceil(frand_a_b(tic*10, tic*20)*PersoCarac[Joueur][FORCE]));
			Attaque(DEGATS, CibleProche(Joueur), Joueur);
		}
		PersoCarac[Joueur][MANA]++;
		break;
...
void AttaquerCHASSEUR(int Joueur) {
	...
	case 0:
		...
		if (AttaquePossible(Joueur, PositionJoueur(CibleAleatoire))) {
			DEGATS = static_cast<int>(ceil(frand_a_b(1.0, 3.0)*PersoCarac[Joueur][FORCE]));
			Attaque(DEGATS, CibleAleatoire, Joueur);
		}
		if (AttaquePossible(Joueur, PositionJoueur(CibleAleatoire))) {
...
void AttaquerBRUTE(int Joueur) {
	...
	case 0:
		...
		DEGATS = static_cast<int>(ceil(frand_a_b(0.5, 1.0)*PersoCarac[Joueur][FORCE]));
		Attaque(DEGATS, CibleAlea(Joueur), Joueur);
		if (AttaquePossible(Joueur, PositionJoueur(CibleProche(Joueur)))) {
			DEGATS = static_cast<int>(ceil(frand_a_b(0.5, 1.0)*PersoCarac[Joueur][FORCE]));
			Attaque(DEGATS, CibleAlea(Joueur), Joueur);
		}
		if (AttaquePossible(Joueur, PositionJoueur(CibleProche(Joueur)))) {
...
void InitialiserNiveau(int Joueur) {
	int i=0;
	while (PersoCarac[Joueur][EXP]>TabXP[i]) {
		i++;
	}
...
void InitialiserUnJoueur(string  NomPersonnage,int PersonnageID,int AGILITE_ID,int FORCE_ID,int VITALITE_ID,int ARME_ID,int CHANCEDOUBLEATTAQUE_ID,int HABILETE_ID,int REDUCTION_ID,int ANIMAL_ID=rand_a_b(0,2)) {
...
int SommeCaracXp(int Joueur) {
	return static_cast<int>(PersoCarac[Joueur][FORCE]+PersoCarac[Joueur][AGILITE]+PersoCarac[Joueur][VITALITE]);
}
...
int SommeStatsPlusExp(int Joueur) {
	return static_cast<int>(ceil((PersoCarac[Joueur][SOINS]+PersoCarac[Joueur][SUBIS]+PersoCarac[Joueur][DEGATS])/100.0));
}
...
bool InitialiserEXP(const string& fichier) {
	...
	ofstream os(fichier);
	long long int K;
	for(int i=0;i<LVLEXP;i++) {


		K= static_cast<long long int>(30*i+10*pow(1.00342,i)-30+i*i);
		os<<K<<endl;
		TabXP[i]=K;
	}
...
void affichageUnJoueur(int IndiceJoueur) {
	...
	cout<<"\tReduction Degats :"<<PersoCarac[IndiceJoueur][REDUCTION]<<"%"<<endl<<endl;
}
...
void InitialiserToutJoueurs() {
	InitialiserUnJoueur("FABIAN",FABIAN,7,7,4,TRUGDOR,20,20,0,OFFENSIF);
	InitialiserUnJoueur("THOMAS",THOMAS,1,4,20,VENTRE,20,80,80,DEFENSIF);
	InitialiserUnJoueur("CLOE",CLOE,11,5,3,BATON,25,10,0, DEFENSIF);
	InitialiserUnJoueur("MAELLE",MAELLE,2,8,8,AMAELLE,50,50,50, DEFENSIF);
	InitialiserUnJoueur("NICOLAS",NICOLAS,8,8,8,NINI,20,20,40, OFFENSIF);
	InitialiserUnJoueur("JB",JB,7,7,7,JBI,1,1,70, DEFENSIF);
	InitialiserUnJoueur("SEBASTIEN",SEBASTIEN,15,2,2,SPARTAN,30,15,0, OFFENSIF);
	InitialiserUnJoueur("FLO",FLO,2,5,13,ALACOOL,10,30,10, DEFENSIF);
	InitialiserUnJoueur("BRYAN",BRYAN,9,9,1,DAGUES,33,33,-33, OFFENSIF);
	InitialiserUnJoueur("SALIM",SALIM,4,4,12,ASALIM,10,10,10, DEFENSIF);
	InitialiserUnJoueur("VICTOR", VICTOR, 1,18,1, KUNGFU, 10, 80, 60, DEFENSIF);
	InitialiserUnJoueur("ISIDORE", ISIDORE, 15, 1, 5, ISIDOOR, 10, 10, 30, DEFENSIF);
	InitialiserUnJoueur("DEMO", DEMO, 5, 5, 10, ADEMO, 0, 10, 0, DEFENSIF);
}
...
void InitialiserCampagneIa( int  Zone, int  Niveau) {
	...
	switch(Zone) {
		case 1:
				...
				switch(Niveau) {
					...
					case 2:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=5;InitialiserUnJoueur("LAPIN ADULTE" ,S3,8,2,2,LAPIN,15,0,10, OFFENSIF);
					...
					case 3:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=3;InitialiserUnJoueur("LAPIN ADULTE1 ",S1,8,2,2,LAPIN,15,0,5, OFFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=4;InitialiserUnJoueur("GROS LAPIN ",S3,5,5,5,LAPIN,0,25,25, OFFENSIF);
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=3;InitialiserUnJoueur("LAPIN ADULTE2 ",S5,8,2,2,LAPIN,15,0,5, OFFENSIF);
						break;
					case 4:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=6;InitialiserUnJoueur("LAPIN ADULTE1" ,S1,8,2,2,LAPIN,15,0,5, OFFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=6;InitialiserUnJoueur("GROS LAPIN ",S3,5,5,5,LAPIN,0,25,25, OFFENSIF);
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=6;InitialiserUnJoueur("LAPIN ADULTE2 ",S5,8,2,2,LAPIN,15,0,5, OFFENSIF);
						break;
					case 5:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=7;InitialiserUnJoueur("SANGLIER MAl" ,S1,6,8,15,SANGLIER,15,25,25,DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=7;InitialiserUnJoueur("SANGLIER FEMELLE ",S2,9,6,12,SANGLIER,25,15,20, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=15;InitialiserUnJoueur("Lapin Agile ",S3,15,1,1,LAPIN,50,50,0, OFFENSIF);
					...
					case 6:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=10;InitialiserUnJoueur("Cerf" ,S1,10,10,10,CERF,15,30,5, OFFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=20;InitialiserUnJoueur("Lapin Agile" ,S3,15,1,1,LAPIN,50,50,0, OFFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=12;InitialiserUnJoueur("LAPIN Chasseur" ,S4,6,6,5,LAPIN,0,30,10, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=10;InitialiserUnJoueur("Cerf ",S5,10,10,10,CERF,15,30,5, OFFENSIF);
						break;
					case 7:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=12;InitialiserUnJoueur("Sanglier féroce ",S1,5,12,12,SANGLIER,10,40,10, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=13;InitialiserUnJoueur("Sanglier colosse ",S2,3,9,18,SANGLIER,0,10,30, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=22;InitialiserUnJoueur("Cerf volant",S3,15,5,5,CERF,0,50,10, OFFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=17;InitialiserUnJoueur("Lapinou ",S4,5,1,5,LAPIN,100,100,90);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=17;InitialiserUnJoueur("Cerf d'acier" ,S5,3,15,15,CERF,0,40,50);
						break;
					case 8:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=20;InitialiserUnJoueur("Lapin d'or ",S1,10,1,25,LAPIN,0,0,80, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=15;InitialiserUnJoueur("Oeuf de paque vivant1 ",S2,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=15;InitialiserUnJoueur("Oeuf de paque vivant2 ",S3,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=15;InitialiserUnJoueur("Oeuf de paque vivant3 ",S4,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=15;InitialiserUnJoueur("Oeuf de paque vivant4 ",S5,4,2,10,LAPIN,0,0,30, DEFENSIF);
						break;
					case 9:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=25;InitialiserUnJoueur("CERF TOI ",S1,1,1,25,CERF,0,0,60, DEFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=30;InitialiserUnJoueur("Roi Des-Cerf ",S3,10,15,10,CERF,50,50,50, DEFENSIF);
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=40;InitialiserUnJoueur("CERF A RIEN ",S5,1,1,10,CERF,1,1,99, DEFENSIF);
						break;
					case 10:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=30;InitialiserUnJoueur("Petit Poney ",S1,30,30,30,PETITPONEY,30,30,30, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=25;InitialiserUnJoueur("Sanglier magique ",S2,5,10,10,SANGLIER,0,25,10, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=25;InitialiserUnJoueur("Cerf enchanté ",S3,10,10,10,CERF,10,10,10, DEFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=50;InitialiserUnJoueur("Dieu Lapin ",S4,3,1,1,LAPIN,5,0,0, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=45;InitialiserUnJoueur("Dieu petit Poney ",S5,30,30,30,PETITPONEY,30,30,30, OFFENSIF);
						break;
					}
			break;
		case 2:
			...
			switch(Niveau) {
					case 1:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=45;InitialiserUnJoueur("LOUP1 ",S1,6,6,4,LAPIN,10,10,0, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=50;InitialiserUnJoueur("LOUP2 ",S2,6,6,4,LAPIN,10,10,0, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=55;InitialiserUnJoueur("CHEF LOUP ",S3,12,12,8,LAPIN,20,20,10, DEFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=50;InitialiserUnJoueur("LOUP3 ",S4,6,6,4,LAPIN,10,10,0, DEFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=45;InitialiserUnJoueur("LOUP4 ",S5,6,6,4,LAPIN,10,10,0, DEFENSIF);
						break;
					case 2:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=60;InitialiserUnJoueur("SANGLIER PROTECTEUR1 ",S1,5,10,10,SANGLIER,5,50,0, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=60;InitialiserUnJoueur("SANGLIER PROTECTEUR2 ",S2,3,15,15,SANGLIER,5,50,25, DEFENSIF);
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=65;InitialiserUnJoueur("CERF PROTECTEUR2 ",S4,3,15,12,CERF,5,75,50, DEFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=75;InitialiserUnJoueur("LAPIN DIVIN ",S5,20,2,2,LAPIN,50,50,50, DEFENSIF);
						break;
					case 3:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
					...
					case 4:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=100;InitialiserUnJoueur("Rattata ",S3,5,5,5,LAPIN,25,25,25, OFFENSIF);
					...
					case 5:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=90;InitialiserUnJoueur("SANGLIER DIVIN ",S1,6,8,15,SANGLIER,15,25,25, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=90;InitialiserUnJoueur("SANGLIER DIVINE ",S2,9,6,12,SANGLIER,25,15,20, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=100;InitialiserUnJoueur("BeBe Dragon ",S3,10,10,10,ADRAGON,10,10,10);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=80;InitialiserUnJoueur("LAPIN DIVIN ",S4,3,1,1,LAPIN,5,0,0, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=80;InitialiserUnJoueur("LAPIN DIVIN ADULTE ",S5,8,2,2,LAPIN,15,0,5, OFFENSIF);
						break;
					case 6:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=100;InitialiserUnJoueur("Cerf dechu ",S1,10,10,10,CERF,15,30,5, DEFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=125;InitialiserUnJoueur("Lapin Agile ",S3,15,1,1,LAPIN,50,50,0, OFFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=120;InitialiserUnJoueur("LAPIN Chasseur des cieux ",S4,6,6,5,LAPIN,0,30,10, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=100;InitialiserUnJoueur("Cerf dechu 2 ",S5,10,10,10,CERF,15,30,5, OFFENSIF);
						break;
					case 7:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=110;InitialiserUnJoueur("Sanglier DIVIN1",S1,5,12,12,SANGLIER,10,40,10, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=120;InitialiserUnJoueur("Sanglier DIVIN colosse ",S2,3,9,18,SANGLIER,0,10,30, DEFENSIF);
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=170;InitialiserUnJoueur("Lapinou ",S4,5,1,5,LAPIN,100,100,90, DEFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=155;InitialiserUnJoueur("Cerf diamant ",S5,3,15,15,CERF,0,40,99, DEFENSIF);
						break;
					case 8:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						...
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=125;InitialiserUnJoueur("Oeuf de paque vivant1 ",S2,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=125;InitialiserUnJoueur("Oeuf de paque vivant2 ",S3,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=125;InitialiserUnJoueur("Oeuf de paque vivant3 ",S4,4,2,10,LAPIN,0,0,30, DEFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=125;InitialiserUnJoueur("Oeuf de paque vivant4 ",S5,4,2,10,LAPIN,0,0,30, DEFENSIF);
						break;
					case 9:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=160;InitialiserUnJoueur("CERF TOI LEGENDAIRE ",S1,1,1,25,CERF,0,0,60, DEFENSIF);
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=180;InitialiserUnJoueur("Roi Des-Cerf LEGENDAIRE",S3,10,15,10,CERF,50,50,50, DEFENSIF);
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=200;InitialiserUnJoueur("CERF A RIEN LEGENDAIRE ",S5,1,1,10,CERF,0,0,99, DEFENSIF);
						break;
					case 10:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=200;InitialiserUnJoueur("Petit Poney ",S1,30,30,30,PETITPONEY,30,30,30, DEFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=225;InitialiserUnJoueur("Sanglier magique ",S2,5,10,10,SANGLIER,0,25,10, OFFENSIF);
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=150;InitialiserUnJoueur("DRAGON LEGENDAIRE ",S3,30,30,30,ADRAGON,30,30,30);
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=350;InitialiserUnJoueur("Dieu Lapin ",S4,3,1,1,LAPIN,5,0,0, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=215;InitialiserUnJoueur("Dieu petit Poney ",S5,30,30,30,PETITPONEY,30,30,30, DEFENSIF);
						break;
					}
			break;
		case 3:
			...
			switch(Niveau) {
					case 1:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 1 ",S1,25,5,5,LAPIN,10,10,0, OFFENSIF);
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 2 ",S2,25,5,5,LAPIN,10,10,0, OFFENSIF);
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 3  ",S4,25,5,5,LAPIN,10,10,0, OFFENSIF);
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=100;InitialiserUnJoueur("LAPIN DRAGON 4  ",S5,25,5,5,LAPIN,10,10,0, OFFENSIF);
						break;
					case 2:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
					...
					case 3:
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=150;InitialiserUnJoueur("DRAGON PONEY ",S5,30,30,30,PETITPONEY,30,30,30,DEFENSIF);
						break;
					case 4:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=250;InitialiserUnJoueur(" DRAGON JB ",S5,7,7,7,JBI,7,7,7,DEFENSIF);
						break;
					case 5:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						...
						Equipe[TEAM2][DEUXIEME]=S2;PersoCarac[S2][EQUIPE]=TEAM2;PersoCarac[S2][LVL]=300;InitialiserUnJoueur("GRAND DRAGON LAPIN ",S2,30,30,30,LAPIN,30,30,30);
					...
					case 6:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
						Equipe[TEAM2][PREMIER]=S1;PersoCarac[S1][EQUIPE]=TEAM2;PersoCarac[S1][LVL]=250;InitialiserUnJoueur("GRAND DRAGON SANGLIER ",S1,30,30,30,SANGLIER,30,30,30,DEFENSIF);
					...
					case 7:
						...
						Equipe[TEAM2][QUATRIEME]=S4;PersoCarac[S4][EQUIPE]=TEAM2;PersoCarac[S4][LVL]=340;InitialiserUnJoueur("GRAND DRAGON CERF ",S4,30,30,30,CERF,30,30,30,DEFENSIF);
					...
					case 8:
						...
						Equipe[TEAM2][CINQUIEME]=S5;PersoCarac[S5][EQUIPE]=TEAM2;PersoCarac[S5][LVL]=300;InitialiserUnJoueur("GRAND DRAGON PETEUR ",S5,25,25,25,VENTRE,25,16,80,DEFENSIF);
						break;
					case 9:
						cout<<endl<<"Niveau "<<Niveau<<" : "<<endl;
					...
					case 10:
						...
						Equipe[TEAM2][TROISIEME]=S3;PersoCarac[S3][EQUIPE]=TEAM2;PersoCarac[S3][LVL]=500;InitialiserUnJoueur("SPYRO ",S3,26,26,26,ADRAGON,26,26,26);
		...
		case 4:
			...
			case 1:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 500; InitialiserUnJoueur("MUR 1 ", S1, 1, 99, 99, JBI, 10, 10,99, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 500; InitialiserUnJoueur("MUR 2 ", S2, 1, 99, 99, JBI, 10, 10, 99, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 500; InitialiserUnJoueur("MUR ", S3, 1, 99, 99, JBI, 10, 10, 99, DEFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 500; InitialiserUnJoueur("MUR 3 ", S4, 1, 99, 99, JBI, 10, 10, 99, DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 500; InitialiserUnJoueur("MUR 4 ", S5, 1, 99, 99, JBI, 10, 10, 99, DEFENSIF);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
			...
			case 4:
				...
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 530; InitialiserUnJoueur("GRAND DRAGON CLOE ", S5, 30, 30, 30, BATON, 30, 30, 30,OFFENSIF);
				break;
			case 5:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				...
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 525; InitialiserUnJoueur("GRAND DRAGON SEB ", S2, 30, 30, 30, SPARTAN, 30, 30, 30);
			...
			case 6:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 550; InitialiserUnJoueur("GRAND DRAGON VICTOR ", S1, 30, 30, 30, KUNGFU, 30, 30, 30);
			...
			case 7:
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 558; InitialiserUnJoueur("GRAND DRAGON SAGE FABIAN ", S4, 30, 30, 30, TRUGDOR, 30, 30, 30);
			...
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 580; InitialiserUnJoueur("THOMAS THE TANK ENGINE ", S1, 1, 99, 99, ADRAGON, 30, 30, 80);
			...
			case 9:
				...
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 625; InitialiserUnJoueur("GRAND DRAGON NICO LE SURVIVANT ", S4, 30, 30, 30, NINI, 30, 30, 30);
			...
			case 10:
				...
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 700; InitialiserUnJoueur("SPYRO TTT ", S3, 29,29, 29, TELLURIQUE, 29, 29,29);
		...
		case 5:
			...
			case 1:
				...
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 500; InitialiserUnJoueur("EASYDOOR  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 70, 10, 10, ISIDOOR, 50, 50, 50, DEFENSIF);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 600; InitialiserUnJoueur(" PERE DE NICO TRES FURIEUX  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 30, 30, 30, DEMO, 30, 30, 30, DEFENSIF);
				break;
			case 3:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 700; InitialiserUnJoueur(" VICTOR HUGOT  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 10, 70, 10, KUNGFU, 100, 50, 90, DEFENSIF);
			...
			case 4:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 800; InitialiserUnJoueur(" GRAND SALIMALEKUM  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 30, 30, 30, ASALIM, 40, 40, 60, DEFENSIF);
				break;
			case 5:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 900; InitialiserUnJoueur(" BRYAN IN THE KITCHEN  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 44, 44, 2, DAGUES, 75, 75, 0, OFFENSIF);
			...
			case 6:
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1000; InitialiserUnJoueur(" FLO D'ENFER  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 5, 35, 50, ALACOOL, 10, 80, 20, DEFENSIF);
				break;
			case 7:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1100; InitialiserUnJoueur(" SEB LA FRITE  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 70, 10, 10, SPARTAN, 80, 20, 0, OFFENSIF);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1200; InitialiserUnJoueur(" JB LE SOLIDE  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 70, 10, 10, JBI, 50, 50, 99, DEFENSIF);
				break;
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1300; InitialiserUnJoueur(" NICOOLA  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 30, 30, 30, NINI, 50, 50, 50, OFFENSIF);
				break;
			case 10:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1;
				...
				Equipe[TEAM2][TROISIEME] = S1;
				Equipe[TEAM2][QUATRIEME] = S1;
				Equipe[TEAM2][CINQUIEME] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1400; InitialiserUnJoueur(" MAELLE LA TYRANIQUE  (" + to_string(PersoCarac[S1][LVL]) + ")", S1, 35, 35, 20, AMAELLE, 50, 50, 50, DEFENSIF);
				break;
			}
			break;
		case 6:
			...
			case 1:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1000; InitialiserUnJoueur("Elite *  ", S1, 3, 10, 10, ELITE, 10, 10, 10, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1000; InitialiserUnJoueur("Grognard * 1 ", S2, 10, 1, 1, GROGNARD, 10, 0, 0,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1000; InitialiserUnJoueur("Grognard * 2 ", S3, 10, 1,1, GROGNARD, 10, 0, 0, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1000; InitialiserUnJoueur("Grognard * 3  ", S4, 10, 1, 1, GROGNARD, 10, 0, 0, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1000; InitialiserUnJoueur("Grognard * 4  ", S5, 10, 1, 1, GROGNARD, 10, 0, 0, OFFENSIF);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1010; InitialiserUnJoueur("Elite * *  ", S1, 5, 12, 12, ELITE, 20, 20, 20,DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1010; InitialiserUnJoueur("Grognard * * 1 ", S2, 15, 1, 1, GROGNARD, 20, 0, 0,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1010; InitialiserUnJoueur("Grognard * * 2 ", S3, 15, 1, 1, GROGNARD, 20, 0, 0,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1010; InitialiserUnJoueur("Chasseur * ", S4, 8, 8, 1, CHASSEUR, 2, 2, 0,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1010; InitialiserUnJoueur("Grognard * * 3 ", S5, 15, 1, 1, GROGNARD, 20, 0, 0,OFFENSIF);
				break;
			case 3:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1020; InitialiserUnJoueur("HUNTER * 1 ", S1, 2, 20, 20, HUNTER, 5, 20, 20,DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1020; InitialiserUnJoueur("HUNTER * 2 ", S2, 2, 20, 20, HUNTER, 5, 20, 20,DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1020; InitialiserUnJoueur("Chasseur * * 1  ", S3, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1020; InitialiserUnJoueur("Chasseur * * 2 ", S4, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1020; InitialiserUnJoueur("Chasseur * * 3 ", S5, 10, 10, 1, CHASSEUR, 4, 4, 1,OFFENSIF);
				break;
			case 4:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1030; InitialiserUnJoueur("Brute 1 * ", S1, 4, 12, 8, BRUTE, 10, 10, 10,OFFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1030; InitialiserUnJoueur("Brute 2 * ", S2, 4, 12, 8, BRUTE, 10, 10, 10,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1030; InitialiserUnJoueur("Brute 3 * ", S3, 4, 12, 8, BRUTE, 10, 10, 10,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1030; InitialiserUnJoueur("Brute 4 * ", S4, 4, 12, 8, BRUTE, 10, 10, 10,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1030; InitialiserUnJoueur("Brute 5 * ", S5, 4, 12, 8, BRUTE, 10, 10, 10, OFFENSIF);
				break;
			case 5:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1040; InitialiserUnJoueur("HUNTER 1 * * ", S1, 3, 22, 22, HUNTER, 10, 40, 40,DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1040; InitialiserUnJoueur("HUNTER 2 * * ", S2, 3, 22, 22,HUNTER, 10, 40, 40,DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1040; InitialiserUnJoueur("Grognard 1 * * * ", S3, 20, 2, 2, GROGNARD, 40, 10, 10,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1040; InitialiserUnJoueur("Grognard 2 * * * ", S4, 20, 2, 2, GROGNARD, 40, 10, 10,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1040; InitialiserUnJoueur("Grognard 3 * * * ", S5, 20, 2, 2, GROGNARD, 40, 10, 10,OFFENSIF);
				break;
			case 6:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1050; InitialiserUnJoueur("ELITE * * * ", S1, 7, 16, 16, ELITE, 30, 30, 30,DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1050; InitialiserUnJoueur("Grognard 1 * * * * ", S2, 25, 2, 2, GROGNARD, 80, 20, 20,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1050; InitialiserUnJoueur("Grognard 2 * * *  ", S3, 25, 2, 2, GROGNARD, 80, 20, 20,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1050; InitialiserUnJoueur("Chasseur * * * ", S4, 12, 12, 2, CHASSEUR, 8, 8, 2,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1050; InitialiserUnJoueur("Grognard 3 * * * * ", S5, 25, 2, 2, GROGNARD, 80, 20, 20,OFFENSIF);
				break;
			case 7:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1060; InitialiserUnJoueur("Brute 1 * * ", S1, 5, 16, 10, BRUTE, 20, 20, 20,OFFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1060; InitialiserUnJoueur("Brute 2 * * ", S2, 5, 16, 10, BRUTE, 20, 20, 20,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1060; InitialiserUnJoueur("Brute 3 * * ", S3, 5, 16, 10, BRUTE, 20, 20, 20,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1060; InitialiserUnJoueur("Brute 4 * * ", S4, 5, 16, 10, BRUTE, 20, 20, 20,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1060; InitialiserUnJoueur("Chasseur * * * * ", S5, 16, 16, 3, CHASSEUR, 16, 16, 4,OFFENSIF);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1070; InitialiserUnJoueur("Brute 1 * * * ", S1, 7, 21, 13, BRUTE, 30, 30, 30,OFFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1070; InitialiserUnJoueur("Brute 2 * * * ", S2, 7, 21, 13, BRUTE, 30, 30, 30,OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1070; InitialiserUnJoueur("Brute 3 * * * ", S3, 7, 21, 13, BRUTE, 30, 30, 30,OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1070; InitialiserUnJoueur("Chasseur 1 * * * * * ", S4, 20, 20, 5, CHASSEUR,32, 32, 8,OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1070; InitialiserUnJoueur("Chasseur 2 * * * * * ", S5, 20, 20, 5, CHASSEUR, 32, 32, 8,OFFENSIF);
				break;
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1080; InitialiserUnJoueur("Hunter 1 * * * ", S1, 6, 26, 26, HUNTER, 15, 60, 60,DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1080; InitialiserUnJoueur("Hunter 2 * * * ", S2, 6, 26, 26, HUNTER, 15, 60, 60,DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1080; InitialiserUnJoueur("Elite 1 * * * * ", S3, 10, 20, 20, ELITE, 40, 40, 40,DEFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1080; InitialiserUnJoueur("Elite 2 * * * * ", S4, 10, 20, 20, ELITE, 40, 40, 40,DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1080; InitialiserUnJoueur("Grognard * * * * * ", S5, 30, 3, 3, GROGNARD, 100, 30, 30,OFFENSIF);
				break;
			case 10:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1090; InitialiserUnJoueur("Brute 1 * * * * * ", S1, 15, 35, 20,BRUTE, 50, 50, 50,OFFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1090; InitialiserUnJoueur("Brute 2 * * * * * ", S2, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1090; InitialiserUnJoueur("Brute 3 * * * * * ", S3, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1090; InitialiserUnJoueur("Brute 4 * * * * * ", S4, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1090; InitialiserUnJoueur("Brute 5 * * * * * ", S5, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				break;
			}
			break;
		case 7:
			...
			case 1:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1100; InitialiserUnJoueur("Grognard * * * * *  ", S1, 30, 9, 3, GROGNARD, 100, 30, 30, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1100; InitialiserUnJoueur("Grognard * * * * * 1 ", S2, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1100; InitialiserUnJoueur("Grognard * * * * *  2 ", S3, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1100; InitialiserUnJoueur("Grognard * * * * * 3  ", S4, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1100; InitialiserUnJoueur("Grognard * * * * * 4  ", S5, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				break;
			case 2:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1110; InitialiserUnJoueur("Elite * * * * *  ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1110; InitialiserUnJoueur("Elite * * * * * 1 ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1110; InitialiserUnJoueur("Grognard * * * * * 2 ", S3, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1110; InitialiserUnJoueur("Grognard * * * * *  ", S4, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1110; InitialiserUnJoueur("Grognard * * * * * 3 ", S5, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				break;
			case 3:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1120; InitialiserUnJoueur("HUNTER * * * * * 1 ", S1, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1120; InitialiserUnJoueur("HUNTER * * * * * 2 ", S2, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1120; InitialiserUnJoueur("Chasseur * * * * * 1  ", S3, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1120; InitialiserUnJoueur("Elite * * * * * 1 ", S4, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1120; InitialiserUnJoueur("Elite * * * * * 2 ", S5, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				break;
			case 4:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1130; InitialiserUnJoueur("Brute 1 * * * * *", S1, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1130; InitialiserUnJoueur("Brute 2 * * * * *", S2, 15, 35, 20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1130; InitialiserUnJoueur("Brute 3 * * * * * ", S3, 15, 35,20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1130; InitialiserUnJoueur("Brute 4 * * * * * ", S4, 15, 35,20, BRUTE, 50, 50, 50, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1130; InitialiserUnJoueur("Brute 5 * * * * * ", S5, 15, 35,20, BRUTE, 50, 50, 50, OFFENSIF);
				break;
			case 5:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1140; InitialiserUnJoueur("Elite 1 * * * * * ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1140; InitialiserUnJoueur("Elite 2 * * * * * ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1140; InitialiserUnJoueur("Grognard 1 * * * * * ", S3, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1140; InitialiserUnJoueur("Chasseur  * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1140; InitialiserUnJoueur("Grognard 2 * * * * * ", S5, 30, 9, 3, GROGNARD, 100, 30, 30, OFFENSIF);
				break;
			case 6:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1150; InitialiserUnJoueur("ELITE * * * * * 1 ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1150; InitialiserUnJoueur("ELITE * * * * * 2", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1150; InitialiserUnJoueur("ELITE * * * * * 3 ", S3, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1150; InitialiserUnJoueur("ELITE * * * * * 4 ", S4, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1150; InitialiserUnJoueur("Chasseur * * * * ", S5, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				break;
			case 7:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1160; InitialiserUnJoueur("Hunter 1 * * * * * ", S1, 14,40,40,HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1160; InitialiserUnJoueur("Hunter 2 * * * * * ", S2, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1160; InitialiserUnJoueur("Chasseur 1 * * * * * ", S3, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1160; InitialiserUnJoueur("Chasseur 2 * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1160; InitialiserUnJoueur("Chasseur * * * *  * ", S5,25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				break;
			case 8:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1170; InitialiserUnJoueur("Hunter 1 * * * * * ", S1, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1170; InitialiserUnJoueur("Hunter 2 * * * * * ", S2, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1170; InitialiserUnJoueur("Hunter 3 * * * * * ", S3, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1170; InitialiserUnJoueur("Hunter 4 * * * * * ", S4, 14, 40, 40, HUNTER, 25, 99, 99, DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1170; InitialiserUnJoueur("Grognard  * * * * * ", S5, 30, 9, 3,GROGNARD, 100, 30, 30, OFFENSIF);
				break;
			case 9:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1180; InitialiserUnJoueur("Elite 4 * * * * * ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1180; InitialiserUnJoueur("Elite 3 * * * * * ", S2, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1180; InitialiserUnJoueur("Elite 1 * * * * * ", S3, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1180; InitialiserUnJoueur("Elite 2 * * * * * ", S4, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1180; InitialiserUnJoueur("Elite 5 * * * * * ", S5, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				break;
			case 10:
				cout << endl << "Niveau " << Niveau << " : " << endl;
				Equipe[TEAM2][PREMIER] = S1; PersoCarac[S1][EQUIPE] = TEAM2; PersoCarac[S1][LVL] = 1190; InitialiserUnJoueur("Elite 1 * * * * * ", S1, 15, 30, 30, ELITE, 50, 50, 50, DEFENSIF);
				Equipe[TEAM2][DEUXIEME] = S2; PersoCarac[S2][EQUIPE] = TEAM2; PersoCarac[S2][LVL] = 1190; InitialiserUnJoueur("Chassaur 1 * * * * * ", S2, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][TROISIEME] = S3; PersoCarac[S3][EQUIPE] = TEAM2; PersoCarac[S3][LVL] = 1190; InitialiserUnJoueur("Chassaur 2 * * * * * ", S3, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][QUATRIEME] = S4; PersoCarac[S4][EQUIPE] = TEAM2; PersoCarac[S4][LVL] = 1190; InitialiserUnJoueur("Chassaur 3 * * * * * ", S4, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				Equipe[TEAM2][CINQUIEME] = S5; PersoCarac[S5][EQUIPE] = TEAM2; PersoCarac[S5][LVL] = 1190; InitialiserUnJoueur("Chassaur 4 * * * * * ", S5, 25, 25, 5, CHASSEUR, 40, 40, 20, OFFENSIF);
				break;
			}
			break;
		case 8:
...
void JouerPartie(int Z, int N) {
	...
	AffichageEquipe();
	int indice=0;
	while(TeamPerdue(TEAM1)&&TeamPerdue(TEAM2)) {
		Tour++;
		...
		if(rand_a_b(0,101)<=1&&(TeamPerdue(TEAM1)&&TeamPerdue(TEAM2))) {
			cout<<" CHANCE !!! "<<endl;
			choixArme(QuiJoue[indice]);
		}
		AffichageEquipe();
	...
	ajouterEXPToutJoueurs(Equipe[TEAM1][PREMIER],Equipe[TEAM1][DEUXIEME],Equipe[TEAM1][TROISIEME],Equipe[TEAM1][QUATRIEME],Equipe[TEAM1][CINQUIEME],Equipe[TEAM2][PREMIER],Equipe[TEAM2][DEUXIEME],Equipe[TEAM2][TROISIEME],Equipe[TEAM2][QUATRIEME],Equipe[TEAM2][CINQUIEME],TeamPerdue(TEAM1));
...
void AfficherMiseAjour() {
	cout<<endl<<"* * * MISE A JOUR 2.08 * * * "<<endl;
	cout<<endl<<"--> Refonte de Maelle , un personnage que personne ne voudra jouer ."<<endl;
	cout << endl << "--> Ajouter d'un nouveau personnage , ISIDORE , reloue comme dans la vrai vie ." << endl;
	cout << endl << "--> Ajouter d'un nouveau personnage , DEMO , capacite de faire revivre les morts ." << endl;
	cout << endl << "--> Augmentation des dégats de l'attaque ultime de Victor ." << endl;
	cout << endl << "--> Les animaux offensif font bien plus mal ." << endl;
	cout << endl << "--> V.10 Zone 5 remplie de BOSS ." << endl;
	cout << endl << "--> V.11 Zone 6 et zone 7 plus facile que la zone 5 normalement voire meme zone 4 ." << endl;
	cout << endl << "--> V.11 Le loot d'orbe ne s'affiche plus quand on possède déja l'orbe, une grosse pause est mise si vous lootez une orbe legendaire ." << endl;
